...
// // File: lib/features/home/presentation/views/home_view.dart (Fixed DashboardTab)
// import 'package:flutter/material.dart';
// import 'package:flutter_riverpod/flutter_riverpod.dart';
...
//   Widget build(BuildContext context, WidgetRef ref) {
//     final user = ref.watch(authStateProvider).value;
//     final userRecipeCount = ref.watch(userRecipeCountProvider(user?.uid));
...
//               if (value == 'logout') {
//                 // FIXED: Show confirmation dialog first
...
//   // FIXED: Add logout confirmation dialog
...
//   // FIXED: Proper logout handling with forced navigation
...
//       // FIXED: Force navigation to login screen immediately
//       if (context.mounted) {
...
// File: lib/features/home/presentation/views/home_view.dart (Patched with Logout Fixes)
...
class DashboardTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final userRecipes =
        recipesState.recipes.where((recipe) => recipe.authorId == user?.uid).toList();
    final userMealPlans =
        mealPlanState.mealPlans.where((plan) => plan.userId == user?.uid).toList();
      ...
      appBar: AppBar(
        ...
        actions: [
          PopupMenuButton(
            onSelected: (value) async {
              if (value == 'logout') {
                // ✅ Changed: Show confirmation dialog first
                final shouldLogout = await _showLogoutDialog(context);
                if (shouldLogout == true) {
                  await _handleLogout(context, ref);
                }
          ...
          children: [
            ...
            todayNutrition.when(
              data: (summary) => nutritionGoal.when(
                data: (goal) {
                  final progress =
                      (summary.totalCalories / goal.caloriesGoal).clamp(0.0, 1.0);
                  return Text(
                    '${summary.totalCalories.toInt()}',
                    style: Theme.of(context).textTheme.headlineMedium?.copyWith(
                          fontWeight: FontWeight.bold,
